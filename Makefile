CMD = scpe
GIT_TAG := $(shell (git describe --abbrev=0 --tags 2> /dev/null || echo $${SCPE_GIT_TAG:-'v0.0.0'}) | head -n1)
GIT_HASH := $(shell (git show-ref --head --hash=8 2> /dev/null || echo $${SCPE_GIT_HASH:-'00000000'}) | head -n1)
